...
- Minima backup, restore, and Megammr resync now release the in-progress lock as soon as they finish instead of relying on a later status poll to notice, so a finished backup could no longer be mistaken for still in progress and falsely block the nightly auto-restart. A Docker error early in a restart attempt now also releases the lock instead of leaving it stuck for up to 6 minutes. The console's raw `restore`/`reset` commands now respect the same lock as every other Minima operation. The health poller no longer burns its resync cooldown when a resync attempt was skipped due to a conflicting operation already in progress. The dashboard no longer shows "restarting…" and freezes status polling when a restart request never actually started.
