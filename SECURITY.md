...
- Automation history is bounded, not permanent: automation runs, block runs, inbox items (including deleted ones), and data source reads older than 30 days or beyond the newest 10,000 rows per table are deleted, at most 500 rows per table at startup and then hourly. Export anything that must be kept longer.
