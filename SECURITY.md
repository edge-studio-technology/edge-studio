...
- Workflow execution diagnostics are bounded, not permanent: automation runs and block runs older than 30 days or beyond the newest 10,000 rows per table are deleted in repeated batches of at most 500 direct deletions at startup and hourly, yielding between batches until drained. Active executions and their block runs are protected. Visible automation inbox items and data-source reads are preserved; user-deleted inbox items are physically purged in bounded batches, and Integritas history remains until explicit deletion. Preserved data can grow without bound until a separate product lifecycle adds configuration, export, quotas, and disk warnings. See [`docs/adr/0023`](docs/adr/0023-classify-stored-records-before-applying-retention.md).
