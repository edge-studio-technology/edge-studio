...
- Never enter admin credentials, import a wallet seed phrase, or download/upload a Minima node backup file over a network connection you cannot verify, even though it is TLS-encrypted. A self-signed certificate proves encryption, not server identity. A node backup file contains the same key material as a seed phrase, plus coin proofs and transaction history, protected only by the single admin-chosen backup password stored in Integritas Pi (not a Minima node/wallet password) that encrypts every backup — treat it with the same care as the seed phrase itself.
