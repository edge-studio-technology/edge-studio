...
- Automation history is bounded, not permanent: automation runs, block runs, inbox items (including deleted ones), and data source reads older than 30 days or beyond the newest 10,000 rows per table are deleted, in repeated batches of at most 500 direct deletions per table at startup and then hourly, yielding between batches until drained. Active executions and their block runs are protected. The row target can be exceeded between sweeps or while executions remain active; retention is not a disk-byte cap. Export anything that must be kept longer.
