...
- Never let `install.sh` take its verifier, its trust anchor, or its verifier runtime from an artifact it is meant to authenticate. The Ed25519 public key, the verifier source, and the pinned Node image digest are embedded in `install.sh` itself; the runtime bundle and the update manifest are both verified against them before anything is extracted or pulled. See [`docs/adr/0016`](docs/adr/0016-install-time-bootstrap-trust-set.md).
