...
`edge-studio` is a Raspberry Pi Docker Compose prototype with a React frontend, TypeScript/Express backend, SQLite persistence, Minima, Integritas stamping, data sources, automation workflows, and a small operational CLI.
...
- Persist runtime state in SQLite under `/data/edge-studio.db`.
