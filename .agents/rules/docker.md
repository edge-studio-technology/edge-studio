...
- Every long-running Compose service uses the shared `x-logging` anchor (`json-file`, `max-size: "10m"`, `max-file: "3"`); mirror it in `scripts/release/build-docker-compose.mjs`, and keep `createBodyFromInspect()` in `update-agent` copying `HostConfig.LogConfig` so updated containers keep rotation. The nginx webhook location logs errors at `crit` only so the token never reaches the error stream. See `docs/adr/0022-bound-external-automation-effects.md`.
