...
- Use existing shared components (`Page`, `Card`, `Pill`, `Modal`, tables/forms helpers) before inventing new ones.
...
- Choose error presentation by scope, not by severity: `ErrorContentState` when a failure leaves a whole table/list/region with nothing to show (render it in place of that content and hide its toolbar/filter/pager chrome), `ErrorAlert` when the page still works and one slice degraded, toast for transient action failures, field `error` for per-control validation. See `docs/adr/0025-error-state-scope-tiers.md`.
- A content region uses the `LoadingState` / `EmptyContentState` / `ErrorContentState` trio, with a retry wherever the load can be re-run. Bare `LoadingDots` / `SpinnerAlt` / `ErrorText` are for inline value placeholders and per-field messages only.
- Pass load-failure copy through `describeLoadFailure()` (`frontend/src/lib/errors.ts`) so bare browser transport text ("Failed to fetch") becomes service-unreachable copy.
...
- Normalize operational/domain errors with `frontend/src/lib/errors.ts` and show details through `frontend/src/components/patterns/ErrorDetailPanel.tsx` when row-level or persisted errors need inspection.
