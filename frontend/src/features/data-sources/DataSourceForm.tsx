  ...
  return (
    <section className="grid min-w-0 gap-3 [&_label]:grid [&_label]:gap-3 [&_label]:font-bold [&_label]:text-slate-700">
      ...
      {type === "webhook" ? (
        ...
        <>
          <label>Capture mode<select value={cameraMode} onChange={(event) => setCameraMode(event.target.value as "photo" | "video")}><option value="photo">Photo</option><option value="video">Video</option></select></label>
          <label>Width<input value={cameraWidth} onChange={(event) => setCameraWidth(event.target.value)} placeholder="1280" inputMode="numeric" /></label>
          <label>Height<input value={cameraHeight} onChange={(event) => setCameraHeight(event.target.value)} placeholder="720" inputMode="numeric" /></label>
