...
export const inputTemplates: DataSourceTemplate[] = [
  ...
  { title: "ESP32 MQTT Sensor", description: "Generate starter firmware for an ESP32 that publishes JSON sensor data", type: "mqtt", config: { brokerUrl: "mqtt://localhost:1883", topic: "sensors/esp32/data", profile: "esp32-sensor" } },
        ...
        {templates.map((template) => {
          ...
          return (
            <Card className="grid gap-3 transition hover:-translate-y-0.5 hover:shadow-[0_12px_28px_rgba(15,23,42,0.08)]" key={template.title}>
              ...
              {template.config.profile === "esp32-sensor" && <MutedText className="m-0">Creates a normal MQTT input source and shows copyable Arduino ESP32 starter firmware after saving.</MutedText>}
