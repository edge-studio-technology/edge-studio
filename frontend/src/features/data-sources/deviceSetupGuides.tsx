...
function bmeSensorGuide(source: DataSource) {
  ...
  return guide(source, `${sensorName} Environmental Sensor Setup`, `Read temperature, humidity, and air pressure from a ${sensorName} module over the Pi I2C bus.`, [
    ...
    ...(source.config.sensor === "bme680" ? [{ title: "BME680 extra pins", body: "Many BME680 breakouts expose six pins because the chip can also speak SPI. Basic I2C still uses VIN, GND, SCL, and SDA, but some boards need SDO and CS/CSB set explicitly.", table: [["SDO", "Optional I2C address select: GND for 0x76, 3.3V for 0x77"], ["CS / CSB", "Tie to 3.3V for I2C mode if your breakout does not already pull it high"]] as Array<[string, string]> }] : []),
