...
/**
 * ESDS: primary | secondary | tertiary (`ghost`) | accent.
...
/**
 * ESDS: Default → md (44px), Compact → sm (32px).
 * TODO(migrate): `xs` is an alias of `sm` — replace call sites with `sm`, then remove `xs`.
 */
type ButtonSize = "md" | "sm" | "xs";
/** ESDS Icon Button: primary | secondary | tertiary (`ghost`) only. */
type IconButtonVariant = "primary" | "secondary" | "ghost";
/** ESDS Icon Button: Default (40px, 20px icon) | Compact (32px, 16px icon). */
...
/** Circle control size + inner glyph size (ESDS Icon Button). */
...
/**
 * Text button. Accessible name comes from `children` (label).
 * Optional `iconStart` / `iconEnd` are decorative beside the label (ESDS 16px slot).
 * TODO(migrate): drop local button class constants and className style overrides; use variant + size.
 * See docs/frontend-design-system.md → Button / IconButton.
 */
...
/**
 * ESDS circular Icon Button. Callers must pass `aria-label` (or `aria-labelledby`).
