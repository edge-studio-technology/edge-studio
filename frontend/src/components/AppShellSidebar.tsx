...
export function AppShellSidebar({
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      setCollapsed(!mq.matches || sidebarStartCollapsedSetting.get());
    };
    sync();
  ...
  // Below EXPAND_MQ an expanded sidebar overlays the page instead of pushing it.
  ...
  return (
    <>
      {narrow ? <div className="w-20 shrink-0" aria-hidden /> : null}
      {overlay ? (
        <div
      ...
      <aside
        ...
        className={cx(
          "bg-surface-inverse text-text-inverse px-margin-tight py-margin-relaxed gap-detail-close top-0 flex h-screen shrink-0 flex-col justify-between overflow-hidden transition-[width] duration-300 ease-in-out",
          narrow ? "fixed left-0 z-40" : "sticky z-30",
          overlay && "shadow-xl",
          collapsed ? "w-20" : "w-80",
        )}
      >
