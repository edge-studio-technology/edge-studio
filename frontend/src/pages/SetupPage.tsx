...
const setupSteps = [
  ...
  { title: "Secure access", description: "Set PIN, local access rules, API keys, and backup options.", complete: false, icon: LockKeyhole },
  {
    ...
    description: "Set up input sources, output targets, and device locations.",
    complete: false,
    icon: RadioTower,
  },
    ...
    >
      <Card className="grid gap-4">
        <div>
          ...
          <MutedText className="m-0 mt-1">
            You have already installed the bundle and opened the dashboard, so setup starts with progress made.
          </MutedText>
        </div>
        <div className="h-3 overflow-hidden rounded-full bg-slate-200">
          <span className="block h-full rounded-full bg-emerald-600" style={{ width: `${(completed / setupSteps.length) * 100}%` }} />
        </div>
      </Card>
      ...
      <section className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(260px,1fr))]">
        {setupSteps.map((step, index) => {
          ...
          return (
            <Card
              className={step.complete ? "grid content-start gap-4 border-emerald-200 bg-emerald-50" : "grid content-start gap-4"}
              key={step.title}
            >
              <div
                className={
                  step.complete
                    ? "grid size-12 place-items-center rounded-[18px] bg-emerald-100 text-emerald-700"
                    : "grid size-12 place-items-center rounded-[18px] bg-slate-100 text-slate-950"
                }
              >
                <Icon size={24} />
              </div>
