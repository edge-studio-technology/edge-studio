...
- Push-source reads (webhook, MQTT, GPIO) store `dataSourceReference(id)` (`data-source:<id>`) as `source_url`, never a webhook URL or broker URL, since those carry credentials. Webhook-token and URL-userinfo redaction lives in `shared/redact.ts`; do not log or persist raw webhook paths elsewhere.
...
- Do not impose arbitrary app-level file/data limits unless required for safety. The Phase 5 resource limits are the safety exception: response-byte, timeout, concurrency, upload, and MQTT-payload caps live in `config/env.ts` via `boundedNumber()`, which clamps rather than honours an out-of-range value. Every new limit follows that shape — configurable in `.env`, generous by default, with a hard maximum an operator cannot exceed. See `docs/adr/0017-outbound-and-upload-resource-limits.md`.
- Operator-supplied URLs (HTTP JSON Source `url`, HTTP JSON Target `url`) go through `backend/src/shared/url-policy.ts` at save time and through `fetchExternalJson()` (`shared/http.ts`) at fetch time. That helper resolves once, rejects if any A/AAAA record is protected, pins the validated address to the socket, and re-checks every redirect hop. Do not add a bare `fetch` on any egress path that takes an API-writable URL. Camera/sensor host helpers are exempt by construction — they read install-time `.env` values, not data-source rows. See `docs/adr/0014-egress-url-policy-for-operator-supplied-urls.md`. `fetchExternalJson()` is also where outbound resource limits are enforced — the decoded-byte response cap, the global concurrency semaphore with its bounded queue (`shared/egress-limiter.ts`), and the clamped deadline — so a new egress path gets them by using it and loses them by not. `fetchJsonWithTimeout()` stays uncapped on purpose: it serves deployment-config callers (Minima RPC, Integritas, status, host helpers) whose responses are legitimately larger.
