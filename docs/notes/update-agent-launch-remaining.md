...
The VPS pull-model deploy plan in [manifest-deploy-pull-model.md](../plans/manifest-deploy-pull-model.md) is superseded — the manifest is now served directly from the public manifest repo via `raw.githubusercontent.com` (see [0008-manifest-served-from-github-raw.md](../adr/0008-manifest-served-from-github-raw.md)); not repeated here.
...
- **Branch protection on `main`**: confirmed still off (`gh api repos/edge-studio-technology/edge-studio/branches/main/protection` → 404). Deferred pending an actual PR-based team workflow; revisit once that's in regular use.
