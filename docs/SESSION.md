...
- Moved manifest, bundle, archive validation, and extraction staging outside `APP_DIR`; release-mode
  ...
  artifact URLs preserve their query when resolving the sibling `.sig` URL. Decision recorded in ADR 0020.
...
Later same branch, table column visibility:

- Added a documented implementation plan for backend-saved table column visibility and recorded that the shared table controls shell should be built first.
- Added the shared `TableControls` pattern and `TableColumnVisibilityButton` modal so every table has a consistent cog-button column chooser location.
...
- Manual browser checks completed across all scoped tables. The one issue found — diagnostics tables keeping a horizontal scrollbar after columns were hidden because `min-w-245` still applied — was fixed by making that wide minimum conditional on visible column count.
...
- Manually browser-check every table column chooser: cog placement, toggles, persistence after refresh/sign-in, last-data-column guard, hidden action/select columns, empty/loading states, and mobile layout.
