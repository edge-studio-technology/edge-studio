...
- Added `docs/guides/tester-device-workflows.md`, a short Pi-vs-PC tester matrix for addable devices/sources/targets, required external hardware, starter workflow blocks, and quick end-to-end pass criteria; linked it from `docs/README.md`. Verified by docs review only.
- Changed the Automation selected-block inspector to render through a fixed portal side sheet, so it reaches the top of the viewport above the workspace chrome; then redesigned its content around the selected block title, cleaner section cards, shared field controls, and shorter action labels. Verified with `npm --prefix frontend run build`.
- Redesigned the Automation watch-mode runtime inspector with section cards for run summary, block status, output JSON, and Diagnostics links, reusing the same inspector/card language as edit mode. Verified with `npm --prefix frontend run build`.
- Extracted reusable workflow rail panel/header components from the toolkit and applied them to watch-mode Run controls, so both right-rail surfaces share container treatment and heading typography.
