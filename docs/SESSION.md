...
- **#258** is closed `Done`/100% with all six checklist items unticked, and its checklist is a manual QA script rather than the unit-test-gap format the other children use. Needs confirming whether it was verified manually or closed by mistake.
- **#259 Node Failure Mode Unit Testing** is the one section of `legacy-ticket-unit-test-gaps.md` left open. It is parented under *Node Management*, not #363, and there is a sibling feature #356 *Node Testing*; it needs an owner. One of its four gaps is already stale — `MinimaPage.test.tsx` now exists.
