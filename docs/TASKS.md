...
- [x] Fixed a blank-frame flicker on any modal-to-modal swap (e.g. delete confirm → progress modal): removed shared `Modal`'s dead `mounted` render gate (`components/ui/Modal.tsx`), which forced every newly-mounted modal to render `null` for one commit — branch `ui/global-style-realignment`, build verified.
