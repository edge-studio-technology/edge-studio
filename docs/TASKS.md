...
- [ ] Manual verification of `feature/minima-resync-rescue-improvements`'s code-review fixes against a real/test Minima node: the new `409` conflict responses on overlapping restart/backup/restore/resync, the rejected-`"`-in-backup-password validation, and the restored `minima_mdsenable: "false"` behavior on the new `minimacore` image — this session's fixes were build/typecheck-verified only.
- [ ] Manual verification of the second sanity-pass fixes on `feature/minima-resync-rescue-improvements` against a real/test node: a completed backup/restore/resync no longer blocks the next nightly auto-restart, and console `restore`/`reset` now return `409` while another operation is in progress — build/typecheck-verified only.
- [ ] Dedupe `runNightlyTick`'s two structurally identical try/catch blocks (backup then restart) in `minima-backup-scheduler.service.ts`, and the resync cooldown math duplicated between `canAutoResync()` (`minima-monitoring.ts`) and the scheduler's own 48h check.
- [ ] Fix `createBackup`'s extra post-RPC `fs.readdir`+`fs.stat` rescan (`minima-backup.service.ts`) — regressed from a prior single pre-RPC `listBackups()` call.
- [ ] Dedupe the frontend hand-rolled 3s polling loop in `MinimaPage.tsx`'s `refreshAfterOperation` against `useMinimaStatusRefresh`'s own restarting-state cadence; add backoff to `waitForContainerRestart` (`docker.control.ts`) instead of fixed 1s polling for up to 5 minutes.
- [ ] Drop the unrelated pure-formatting diff in `SetupPage.tsx` from this branch (zero logic change, unrelated to the Minima work).
- [ ] Dedupe the five near-identical confirm-password modals in `MinimaBackupPanel.tsx` (~250 lines) into one shared component — flagged during code review, deliberately deferred as a separate, more careful pass.
- [ ] Dedupe `MinimaBackupError`/`MinimaConsoleError` error classes and their duplicate re-auth-check logic (`minima-backup.service.ts` vs `minima-console.service.ts` — the latter is missing the former's explicit `!userId` guard) — flagged during code review, deliberately deferred alongside the modal dedup above since both touch auth-adjacent flows.
...
- [x] Ran an 8-agent parallel code-review + security-review of `feature/minima-resync-rescue-improvements`'s diff since `main`; fixed a concurrency race between Minima restart/backup/restore/resync (now `409` on overlap via a new `MinimaOperationConflictError`), an `endMinimaOperation()` leak on a transient Docker error during restart, unescaped-quote backup-password RPC corruption, the auto-restart scheduler's optimistic cooldown timestamp + missing audit log, a silently-dropped `minima_mdsenable: "false"` from the `minimacore` image swap, a stale frontend post-restart polling timeout, and three code-duplication findings (bool-setting helper, compose-service start/restart, `formatSize`) — see `docs/SESSION.md` for the full list and deferred items.
- [x] Ran a second sanity-pass multi-agent code-review + security-review of `feature/minima-resync-rescue-improvements`'s full git history (excluding merges from `main`) to check whether commit `7f53b57` fully closed out the prior pass's concurrency-lock findings — it hadn't. Fixed 5 must-fix items: backup/restore/resync now release the operation lock on their own success path instead of relying on a later status poll; `restartMinimaContainer()` now wraps `getComposeServiceContainer` in try/catch (the one call `7f53b57` missed next to the one it fixed); the console's raw `restore`/`reset` passthrough commands now respect the operation lock; the health poller no longer burns its resync cooldown on a lock-conflict skip; the dashboard no longer shows "restarting…" when a restart request never actually started — see `docs/SESSION.md` for the full list and deferred cleanup items.
