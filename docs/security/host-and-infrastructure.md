...
- Backups are tracked in two lists by trigger source, not encryption status (every backup now uses the same real password): Manual (capped at 5, creation blocked with a clear error at the cap) and Auto (capped at 10, oldest deleted automatically). Both are downloadable and restorable.
- Automatic backups are created by the backend's own scheduler (`minima-backup-scheduler.service.ts`, every 24h) rather than Minima's built-in `backup auto:true` — verified against Minima's own source (`minima-global/Minima`) that the built-in scheduler always uses Minima's hardcoded default password (`"minima"`, public) and always writes to Minima's own default data folder with no configurable path, making it unable to deliver a real password or a visible/capped location for recurring runs. Enabling the backend's own scheduler requires a backup password to already be set.
