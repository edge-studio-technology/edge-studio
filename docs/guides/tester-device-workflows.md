...
| `BME280 Environmental Sensor` | BME280 I2C module, jumper wires, I2C enabled on the Pi host, `I2C sensors` enabled from Devices -> Hardware support | `Manual run` or `Schedule` -> `Fetch data source` -> `Show preview`; attach `Stamp data` to `Fetch data source` |
| `BME680 Environmental Sensor` | BME680 I2C module, jumper wires, I2C enabled on the Pi host, `I2C sensors` enabled from Devices -> Hardware support; some boards also need `SDO` address select and `CS/CSB` tied for I2C mode | `Manual run` or `Schedule` -> `Fetch data source` -> `Show preview`; attach `Stamp data` to `Fetch data source` |
| `Raspberry Pi Camera` | Pi camera module or USB camera exposed to the Pi; camera support enabled from Devices -> Hardware support | `Manual run` or `Schedule` -> `Capture camera` -> `Show preview`; attach `Stamp data` to `Capture camera` |
